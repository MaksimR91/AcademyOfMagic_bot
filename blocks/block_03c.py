...
def handle_block3c(message_text, user_id, send_reply_func, client_request_date):
...
# ===== Переходы в другие блоки =====
    ...
    # ===== Продолжаем диалог =====
    update_state(user_id, {"stage": "block3c", "last_message_ts": time.time(), "last_bot_question": reply})
    plan(user_id,
    "blocks.block_03c:send_first_reminder_if_silent",   # <‑‑ путь к функции
